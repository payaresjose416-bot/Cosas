...
**The Excel is never the source of truth for the app's live stock.** The user registers every salida/entrada through Registro, and `stock` is a running total the app keeps entirely on its own — an early version of this feature (`detectStockSync`, now removed) read the Excel's `Restantes` column and overwrote `stock` with it, which sounds reasonable until you notice `Restantes` is itself a formula over the Excel's own daily-consumption columns (`SUM(F:AR)` or similar) — columns the user never fills in, because they track consumption in the app instead. That formula just echoes back the stock inicial, stale, forever. Syncing from it doesn't correct drift, it **reintroduces** it, silently clobbering real registered consumption (confirmed live: a product with 5 registered salidas reverted to its pre-salida quantity after one sync). `detectInitialStockSync` is the replacement — it reads **only** product names and the `Stock inicial` column, and writes exclusively to `initialStocks` (via `setInitialStock`/`applyInitialStockSync`), never to `stock`. If you're tempted to re-add a path that writes Excel data into `stock`, don't — there isn't a column in this workbook that safely represents current stock; the app's own registro is definitionally more current than anything in the file the user uploads.
...
`initialStocks` follows the same override pattern as `thresholds`: a synced `{productId: {value, updatedAt}}` map (tombstone-deletable) that shadows the catalog's static `product.initialStock` when present. `getInitialStock(productId)` in `useInventory.js` resolves the override or falls back to the catalog value — this is what the Dashboard's "stock inicial" editor, and `detectInitialStockSync`, both read/write via `setInitialStock`. It's purely a reference figure shown alongside current stock, not itself the seed for `stock` (that seeding already happened at `initStockEntries`/product-creation time) — current stock only ever moves via registro (`saveDay`) or a manual `updateStock` edit.

Some devices' synced `history` may still contain legacy entries with `type: 'sync'` (old Excel-driven stock overwrites, before this was fixed) — `TabHistorial.jsx` still renders and can delete them (as an oldStock→newStock audit line, distinct styling), and `stockBumpForDeleteDay` in `core/inventory.js` knows not to try to reverse a stock delta for them (they never held one in the new model, but old ones do). Nothing creates new entries of this type anymore.
