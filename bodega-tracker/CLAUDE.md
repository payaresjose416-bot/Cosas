...
A legacy cloud key `'stock'` still exists from the old stored-counter model — nothing writes to it anymore, but `useInventory.js`'s and `cli/lib/store.js`'s startup migration reads it **once** (only for products whose `initialStocks` entry has no `date` yet) to seed the anchor losslessly: `value = <the last qty the old model had>, date = today`. This preserves every product's number across the migration without requiring a version bump or a coordinated deploy — never delete that migration path casually, and never repurpose the `'stock'` key for anything new.
