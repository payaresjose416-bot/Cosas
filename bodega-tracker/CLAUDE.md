...
`matchProduct` is used for writing consumption back into the Excel (`writeToExcel`), for `utils/excelDetect.js` (finding brand-new product names not yet in the catalog), and for `utils/excelStockSync.js` (`detectStockSync`). All of these expect the sheet named `Matriz de Consumo (2)` and its fixed layout (name in column B, dates starting column F, data starting row 3).

`detectStockSync` reads the **closing** stock column (`Restantes`/`Saldo`/`Stock actual`), scanning the first few rows for the header and explicitly rejecting any header that also says `inicial`/`entrada`/`compra` — reading the cycle's *opening* stock instead of its remainder silently repopulates the dashboard with month-old numbers. Because that column is calculated, its cells are usually formulas; a workbook written by a script (openpyxl) stores the formula with **no cached value**, so `readStockValue` falls back to the formatted text (`.w`) and, failing that, reports the row in `skipped` with a reason instead of dropping it. Never reintroduce a bare `continue` there: every skipped row is a product whose stock stays silently stale, and both the Exportar tab and `bodega excel sync-stock` surface `skipped` to the user for that reason.
