...
- **`useBuzon.js`** — owns `notes` (cloud key `buzon`), an inbox an agent driving the CLI (`bodega queja`) can leave findings in for the user; the web app can only dismiss (tombstone) a note, never create one — creation is CLI-only, see `plugins/bodega/skills/bodega/SKILL.md`.
...
`useSync.js`'s initial reconciliation (`reconcileFromCloud`) does exactly **one** `loadFromCloud` and computes the merge itself instead of delegating to `syncToCloud` — an earlier version called `syncToCloud(localValueRef.current)` after already reading the cloud once, which made `syncToCloud`'s own read-merge-write do a **second**, uncoordinated read of the same row. Two near-simultaneous `SELECT`s have no ordering guarantee; if the second happened to return an older snapshot than the first, the merge kept the stale local value and wrote it straight back, silently reverting a fresh write from another device (e.g. the CLI) moments after it landed. Same bug class as the startup-migration story above — don't reintroduce a second cloud read inside the initial-load path. The same hook also re-runs `reconcileFromCloud` on `visibilitychange`/`focus`/`pageshow` and every 5 minutes while visible, since a backgrounded PWA can sit for a long time without remounting React or keeping its realtime WebSocket alive.
...
- **Buzón** — read-only list of notes left via `bodega queja` in the CLI, newest first; each can be dismissed (tombstoned) but not created from here.
